...
template <typename Comparable>
void SplayTree<Comparable>::printTree(BinaryNode *t, bool flag) const
{
    if (t != t->left)
    {
...
{
    BinaryNode *rotNode = x->left;
    x->left = rotNode->right;
    rotNode->right = x;
    rotNode = x;
}
...
{
    BinaryNode *rotNode = x->right;
    x->right = rotNode->left;
    rotNode->left = x;
    x = rotNode;
}
...
int SplayTree<Comparable>::remove(const Comparable &x)
{
    // If x is found, it will be splayed to the root by contains
        ...
        else
        {
            BinaryNode *newTree;
            theSize--;
            Comparable retValue = root->value;
